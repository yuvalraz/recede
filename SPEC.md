...
**A second reference profile: `recede/ref-weighting-v0.2`.** Where v0.1 collapses a Warrant's checks to a flat mean confidence (so one weak VALIDATE can drag a strong VERIFY below its own weight), v0.2 pools corroborating evidence with a class-deduped noisy-OR combiner. Each check carries a primary evidence descriptor (its `evidence_refs`) that maps to a declared per-class, per-tier weight; VERIFY is weightable (no longer pinned at 1.0); two anti-gaming gates floor a test-class check without mutation evidence and a self-authored check to the weakest tier. The per-check primary descriptor drives that check's weight, and pooling is cross-check (distinct classes compound, redundant same-class evidence does not). The declared weight magnitudes are policy an adopter authors, not a prediction Recede ships. This profile keeps I2, I4, and I5: the negative fold path, forced demotion, and sample_count are inherited verbatim from v0.1, and only the positive confidence is pooled. Its conformance vector is `conformance/vectors-v0.2.json`.

**Cross-conformance.** Two implementations are **cross-conformant** iff, **under the same declared weighting profile**, they replay the same ordered Warrants to the same TrustState (score to `1e-9`, same tier, confidence, and sample_count) and derive the same record `id`s. This is not asserted, it is **demonstrated**: `conformance/vectors.json` fixes a policy and an ordered, explicitly-timestamped sequence of Warrants (covering accrual, recede, a `never_recede` gate, and a deferred REVERTED reseal) together with the expected final TrustState and the expected content-hash of a pinned record. Both reference implementations load that vector and MUST reproduce it exactly. A third implementation joins the conformance set by declaring `recede/ref-weighting-v0.1` and passing the same vector. For `recede/ref-weighting-v0.2`, cross-conformance is currently **TS-demonstrated**: the TypeScript reference replays `conformance/vectors-v0.2.json` to its frozen final TrustState and pinned record `id`, and the Python mirror is a declared **fast-follow** (FF1) whose harness prints an explicit unsupported-profile skip until the port lands.
